...
export default class SalesPartnerNotificationController {
  async index(req: FastifyRequest, res: FastifyReply) {
    try {
      const userId = (req as any).user?.id;
      const language = (req as any).language;
      const notifications =
        await new UserNotificationModel().salesPartnerUserNotifications(
          userId,
          language
        );
  ...
  async readNotification(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      // Check if notification belongs to this sales partner user
      if (
        notification.target_id !== userId ||
        notification.target_type !== "sale_partner_users"
      ) {
