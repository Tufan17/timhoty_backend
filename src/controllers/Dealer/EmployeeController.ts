...
export default class DealerUserController {
  async findAll(req: FastifyRequest, res: FastifyReply) {
    try {
      const { search="", page=1, limit=10,startDate,endDate } = req.query as {
        search: string;
        page: number;
        limit: number;
        startDate: string;
      ...
      const dealer_id = (req.user as any).dealer_id;
      const query = knex("dealer_users")
        .leftJoin("dealers", "dealers.id", "dealer_users.dealer_id")
        .where("dealer_users.dealer_id", dealer_id)
      ...
      if (startDate && endDate) {
        query.whereBetween("dealer_users.created_at", [startDate, endDate]);
      }

      if (startDate) {
        query.where("dealer_users.created_at", ">=", startDate);
      }

      if (endDate) {
        query.where("dealer_users.created_at", "<=", endDate);
      }
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      let body: any = {};
      if(!req.user || req.user.id !== dealerUser.id) {
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      if (user_id === id) {
        return res.status(400).send({
          success: false,
          message: "Kullanıcı kendisini silemez",
        });
      }
      const exists = await new DealerUserModel().first({ id: id });
      if (!exists) {
        return res.status(400).send({
          success: false,
      ...
      if(exists.type === "manager") {
        return res.status(400).send({
          success: false,
          message: "Yönetici kullanıcısını silemezsiniz",
        });
      }
     

      if(exists.dealer_id !== (req.user as any).dealer_id) {
        return res.status(400).send({
          success: false,
  ...
  async getEmployee(req: FastifyRequest, res: FastifyReply) {
    try {
      const { id } = req.params as { id: string };
      const dealerUser = await new DealerUserModel().first({ id: id });
      const permissions = await new PermissionModel().getDealerPermissions(id);
      if (!dealerUser) {
        return res.status(400).send({
          success: false,
      ...
      return res.status(200).send({
        success: true,
        message: "Çalışan başarıyla görüntülendi",
        data: {...dealerUser, permissions},
      });
    } catch (error) {
      return res.status(500).send({
        success: false,
        message: "Çalışan görüntüleme hatası",
      });
    }
  }
}
