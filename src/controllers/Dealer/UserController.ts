...
export default class UserController {
  ...
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      const { job_id, limit, page, search,startDate,endDate } = req.query as {
        job_id: string;
        limit: number;
        page: number;
        search: string;
      ...
      const query = knex("users")
        ...
        .modify((queryBuilder) => {
          ...
          if (startDate && endDate) {
            queryBuilder.whereBetween("users.created_at", [startDate, endDate]);
          }else if (startDate) {
            queryBuilder.where("users.created_at", ">=", startDate);
          }else if (endDate) {
            queryBuilder.where("users.created_at", "<=", endDate);
          }
        });
