...
export class VisaPackageController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const query = knex("hotel_room_packages")
				...
				.where(function () {
					if (search) {
						...
						this.where(function () {
							...
							this.where("hotel_room_packages.discount", "ilike", like).orWhere("hotel_room_packages.total_tax_amount", "ilike", like)
				...
				.select("hotel_room_packages.*", knex.raw("json_agg(hotel_room_package_prices.*) as prices"))
				.groupBy("hotel_room_packages.id")

			// Get total count - using a separate query without joins to get accurate count
			const countResult = await knex("hotel_room_packages").whereNull("deleted_at").where("hotel_room_id", hotel_room_id).count("* as total").first()
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
          ...
          `),
				...
				.innerJoin("visa_package_prices", "visa_packages.id", "visa_package_prices.visa_package_id")
				.where("visa_packages.id", id)
				.whereNull("visa_package_prices.deleted_at")
				.whereNull("visa_packages.deleted_at")
				.innerJoin("currency_pivots", "visa_package_prices.currency_id", "currency_pivots.currency_id")
				.where("currency_pivots.language_code", req.language)
				.innerJoin("currencies", "visa_package_prices.currency_id", "currencies.id")
				.groupBy("visa_packages.id", "visa_package_pivots.name", "visa_package_pivots.description", "visa_package_pivots.refund_policy", "currencies.code")
				.first()
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { visa_id, discount, total_tax_amount, constant_price, name, description, refund_policy, return_acceptance_period, prices } = req.body as CreatePackageBody
			...
			await new VisaModel().update(existingVisa.id, {
	...
	async findOnePackage(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const packageModel = await knex.select("visa_packages.*", knex.raw("json_agg(visa_package_prices.*) as prices")).from("visa_packages").leftJoin("visa_package_prices", "visa_packages.id", "visa_package_prices.visa_package_id").where("visa_packages.id", id).whereNull("visa_package_prices.deleted_at").whereNull("visa_packages.deleted_at").groupBy("visa_packages.id").first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const { visa_id, discount, total_tax_amount, constant_price, return_acceptance_period, prices, name, description, refund_policy } = req.body as CreatePackageBody
			...
			// Delete old prices
			await knex("visa_package_prices").del().where("visa_package_id", id).whereNull("deleted_at")
			...
			const updatedPackage = await knex.select("visa_packages.*", knex.raw("json_agg(visa_package_prices.*) as prices")).from("visa_packages").leftJoin("visa_package_prices", "visa_packages.id", "visa_package_prices.visa_package_id").where("visa_packages.id", id).whereNull("visa_package_prices.deleted_at").whereNull("visa_packages.deleted_at").groupBy("visa_packages.id").first()
			await new VisaModel().update(packageModel.visa_id, {
				status: false,
				admin_approval: false,
			})
