...
export default class VisaFeatureController {
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("visa_features.id as total")
        .first();
  ...
  async findOne(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const visaFeature = await knex("visa_features")
        .whereNull("visa_features.deleted_at")
        .where("visa_features.id", id)
        .select(
          "visa_features.*", 
          "visa_feature_pivots.name"
        )
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const updatedVisaFeature = await new VisaFeatureModel().oneToMany(
        id,
        "visa_feature_pivots",
        "visa_feature_id"
      );
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("visa_feature_pivots")
        .where("visa_feature_id", id)
