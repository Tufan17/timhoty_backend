...
export default class VisaPackageFeatureController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("visa_package_features")
				...
				.innerJoin("visa_package_feature_pivots", "visa_package_features.id", "visa_package_feature_pivots.visa_package_feature_id")
				.innerJoin("visa_packages", "visa_package_features.visa_package_id", "visa_packages.id")
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("visa_package_features.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("visa_package_features").whereNull("visa_package_features.deleted_at").select("visa_package_features.*", "visa_package_feature_pivots.name").innerJoin("visa_package_feature_pivots", "visa_package_features.id", "visa_package_feature_pivots.visa_package_feature_id").where("visa_package_feature_pivots.language_code", language).whereNull("visa_package_feature_pivots.deleted_at")

			if (visa_package_id) {
				query = query.where("visa_package_features.visa_package_id", visa_package_id)
			}
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const feature = await knex("visa_package_features").whereNull("visa_package_features.deleted_at").where("visa_package_features.id", id).select("visa_package_features.*", "visa_package_feature_pivots.name").innerJoin("visa_package_feature_pivots", "visa_package_features.id", "visa_package_feature_pivots.visa_package_feature_id").where("visa_package_feature_pivots.language_code", req.language).first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingFeature = await knex("visa_package_features").where("id", id).first()
			...
			if (visa_package_id) {
				const visaPackage = await knex("visa_packages").where("id", visa_package_id).first()
			...
			const updatedFeature = await knex("visa_package_features")
				...
				.select("visa_package_features.*", "visa_package_feature_pivots.name")
				.leftJoin("visa_package_feature_pivots", "visa_package_features.id", "visa_package_feature_pivots.visa_package_feature_id")
				.where("visa_package_feature_pivots.language_code", (req as any).language)
				.first()
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("visa_package_feature_pivots").where("visa_package_feature_id", id).update({ deleted_at: new Date() })
			return res.status(200).send({
				success: true,
