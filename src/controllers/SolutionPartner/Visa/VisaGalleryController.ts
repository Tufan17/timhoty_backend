...
export default class VisaGalleryController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("visa_galleries")
				...
				.innerJoin("visas", "visa_galleries.visa_id", "visas.id")
				...
				.modify(qb => {
					...
					if (search) {
						const like = `%${search}%`
						qb.andWhere(w => {
							w.where("visa_galleries.image_type", "ilike", like).orWhere("visa_gallery_pivot.category", "ilike", like)
						})
					}
				})

			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("visa_galleries.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("visa_galleries").whereNull("visa_galleries.deleted_at").leftJoin("visa_gallery_pivot", "visa_galleries.id", "visa_gallery_pivot.visa_gallery_id").where("visa_gallery_pivot.language_code", language).whereNull("visa_gallery_pivot.deleted_at").select("visa_galleries.*", "visa_gallery_pivot.category")

			if (visa_id) {
				query = query.where("visa_galleries.visa_id", visa_id)
			}
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const image = await knex("visa_galleries").whereNull("visa_galleries.deleted_at").leftJoin("visa_gallery_pivot", "visa_galleries.id", "visa_gallery_pivot.visa_gallery_id").where("visa_gallery_pivot.language_code", language).whereNull("visa_gallery_pivot.deleted_at").where("visa_galleries.id", id).select("visa_galleries.*", "visa_gallery_pivot.category").first()
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			for (const imageUrl of imageUrls) {
				...
				await new VisaModel().update(visa_id, { admin_approval: false, status: false })
			}
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			if (category) {
				await knex("visa_gallery_pivot").where({ visa_gallery_id: id }).update({ deleted_at: new Date() })
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex.transaction(async trx => {
				...
				await trx("visa_galleries").where({ id }).update({ deleted_at: new Date() })

				// Soft delete translations
				await trx("visa_gallery_pivot").where({ visa_gallery_id: id }).update({ deleted_at: new Date() })
			})
	...
	async bulkDelete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingImages = await knex("visa_galleries").whereIn("id", ids).whereNull("deleted_at")
			...
			await knex.transaction(async trx => {
				...
				await trx("visa_galleries").whereIn("id", ids).update({ deleted_at: new Date() })

				// Soft delete translations
				await trx("visa_gallery_pivot").whereIn("visa_gallery_id", ids).update({ deleted_at: new Date() })
			})
