...
export default class VisaOpportunityController {
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const base = knex("visa_opportunities")
        ...
        .innerJoin(
          "visa_opportunity_pivots",
          "visa_opportunities.id",
          "visa_opportunity_pivots.visa_opportunity_id"
        )
        ...
        .modify((qb) => {
          ...
          if (search) {
            ...
            qb.andWhere((w) => {
              w.where(
                "visa_opportunity_pivots.category",
                "ilike",
                like
              ).orWhere("visa_opportunity_pivots.description", "ilike", like);
            });
          }
        });
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("visa_opportunities.id as total")
        .first();
      ...
      const data = await base
        ...
        .select(
          "visa_opportunities.*",
          "visa_opportunity_pivots.category",
          "visa_opportunity_pivots.description",
          "visas.location_id"
        )
  ...
  async findAll(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      let query = knex("visa_opportunities")
        .whereNull("visa_opportunities.deleted_at")
        .select(
          "visa_opportunities.*",
          "visa_opportunity_pivots.category",
    ...
    try {
      ...
      if (visa_id) {
        await new VisaOpportunityModel().update(id, {
          visa_id:
            visa_id !== undefined ? visa_id : existingVisaOpportunity.visa_id,
        });
      }
      ...
      const updatedVisaOpportunity = await new VisaOpportunityModel().oneToMany(
        id,
        "visa_opportunity_pivots",
        "visa_opportunity_id"
      );
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("visa_opportunity_pivots")
        .where("visa_opportunity_id", id)
