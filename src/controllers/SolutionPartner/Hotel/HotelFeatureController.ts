...
export default class HotelFeatureController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("hotel_features.id as total").first()
			...
			const data = await base
				...
				.select("hotel_features.*", "hotel_feature_pivots.name", "hotels.location_id")
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("hotel_features").whereNull("hotel_features.deleted_at").select("hotel_features.*", "hotel_feature_pivots.name").innerJoin("hotel_feature_pivots", "hotel_features.id", "hotel_feature_pivots.hotel_feature_id").where("hotel_feature_pivots.language_code", language).whereNull("hotel_feature_pivots.deleted_at")
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const hotelFeature = await knex("hotel_features").whereNull("hotel_features.deleted_at").where("hotel_features.id", id).select("hotel_features.*", "hotel_feature_pivots.name").innerJoin("hotel_feature_pivots", "hotel_features.id", "hotel_feature_pivots.hotel_feature_id").where("hotel_feature_pivots.language_code", language).whereNull("hotel_feature_pivots.deleted_at").first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const { hotel_id, name, status } = req.body as {
				hotel_id?: string
				name?: string
				status?: boolean
			}
			...
			const updatedHotelFeature = await new HotelFeatureModel().oneToMany(id, "hotel_feature_pivots", "hotel_feature_id")

			await new HotelModel().update(existingHotelFeature.hotel_id, { admin_approval: false, status: false })
			return res.status(200).send({
				success: true,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("hotel_feature_pivots").where("hotel_feature_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
