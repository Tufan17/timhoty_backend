...
export default class HotelController {
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const hotels = await knex("hotels").whereNull("hotels.deleted_at").select("hotels.*", "hotel_pivots.name as name", "hotel_pivots.general_info", "hotel_pivots.hotel_info", "hotel_pivots.refund_policy").innerJoin("hotel_pivots", "hotels.id", "hotel_pivots.hotel_id").where("hotel_pivots.language_code", language)
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const { location_id, pool, star_rating, private_beach, transfer, map_location, free_age_limit, solution_partner_id, status, highlight, refund_days, name, general_info, hotel_info, refund_policy } = req.body as {
				location_id?: string
				pool?: boolean
				private_beach?: boolean
				transfer?: boolean
			...
			let body: any = {
				...
				star_rating: star_rating !== undefined ? star_rating : existingHotel.star_rating,
			}
			...
			await new HotelModel().update(id, { admin_approval: false, status: false })
			return res.status(200).send({
				success: true,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("hotel_pivots").where("hotel_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
	...
	async sendForApproval(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let hotelRoomOpportunities = await new HotelRoomOpportunityModel().exists({ hotel_room_id: hotelRoom?.id })
			...
			if (hotel && hotelRoom ? true : false && hotelGalleries && hotelOpportunities && hotelFeatures && hotelRoomOpportunities && hotelRoomFeatures && hotelRoomImages && hotelRoomPackages) {
