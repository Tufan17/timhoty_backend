...
export default class HotelController {
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const { location_id, pool, private_beach, transfer, map_location, free_age_limit, solution_partner_id, status, highlight, refund_days, name, general_info, hotel_info, refund_policy } = req.body as {
				location_id?: string
				pool?: boolean
				private_beach?: boolean
				transfer?: boolean
