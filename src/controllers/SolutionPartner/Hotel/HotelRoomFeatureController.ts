...
export default class HotelRoomFeatureController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("hotel_room_features")
				...
				.innerJoin("hotel_rooms", "hotel_room_features.hotel_room_id", "hotel_rooms.id")
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("hotel_room_features.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("hotel_room_features").whereNull("hotel_room_features.deleted_at").select("hotel_room_features.*", "hotel_room_feature_pivots.name").innerJoin("hotel_room_feature_pivots", "hotel_room_features.id", "hotel_room_feature_pivots.hotel_room_feature_id").where("hotel_room_feature_pivots.language_code", language).whereNull("hotel_room_feature_pivots.deleted_at")

			if (hotel_room_id) {
				query = query.where("hotel_room_features.hotel_room_id", hotel_room_id)
			}
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const feature = await knex("hotel_room_features").whereNull("hotel_room_features.deleted_at").where("hotel_room_features.id", id).select("hotel_room_features.*", "hotel_room_feature_pivots.name").innerJoin("hotel_room_feature_pivots", "hotel_room_features.id", "hotel_room_feature_pivots.hotel_room_feature_id").where("hotel_room_feature_pivots.language_code", req.language).first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingFeature = await knex("hotel_room_features").where("id", id).first()
			...
			if (hotel_room_id) {
				const hotelRoom = await knex("hotel_rooms").where("id", hotel_room_id).first()
			...
			if (Object.keys(updateData).length > 0) {
				await knex("hotel_room_features").where("id", id).update(updateData)
			}
			...
			const updatedFeature = await knex("hotel_room_features")
				...
				.select("hotel_room_features.*", "hotel_room_feature_pivots.name")
				.leftJoin("hotel_room_feature_pivots", "hotel_room_features.id", "hotel_room_feature_pivots.hotel_room_feature_id")
				.where("hotel_room_feature_pivots.language_code", (req as any).language)
				.first()

			await new HotelModel().update(existingFeature.hotel_room.hotel_id, { admin_approval: false, status: false })
			return res.status(200).send({
				success: true,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("hotel_room_feature_pivots").where("hotel_room_feature_id", id).update({ deleted_at: new Date() })
			return res.status(200).send({
				success: true,
