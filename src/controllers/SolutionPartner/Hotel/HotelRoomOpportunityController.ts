...
export default class HotelRoomOpportunityController {
    ...
    try {
      ...
      const countRow = await base
        ...
        .countDistinct<{ total: string }>(
          "hotel_room_opportunities.id as total"
        )
        .first();
      ...
      const data = await base
        ...
        .select(
          "hotel_room_opportunities.*",
          "hotel_room_opportunity_pivots.name"
        )
  ...
  async findAll(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      let query = knex("hotel_room_opportunities")
        .whereNull("hotel_room_opportunities.deleted_at")
        .select(
          "hotel_room_opportunities.*",
          "hotel_room_opportunity_pivots.name"
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      if (hotel_room_id) {
        operations.push(
          new HotelRoomModel().first({ "hotel_rooms.id": hotel_room_id })
        );
      }
      ...
      if (hotel_room_id) {
        updateOperations.push(
          new HotelRoomOpportunityModel().update(id, { hotel_room_id })
        );
      }
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("hotel_room_opportunity_pivots")
        .where("hotel_room_opportunity_id", id)
