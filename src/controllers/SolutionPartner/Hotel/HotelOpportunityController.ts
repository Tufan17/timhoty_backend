...
export default class HotelOpportunityController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("hotel_opportunities")
				...
				.modify(qb => {
					...
					if (search) {
						const like = `%${search}%`
						qb.andWhere(w => {
							w.where("hotel_opportunity_pivots.category", "ilike", like).orWhere("hotel_opportunity_pivots.description", "ilike", like)
						})
					}
				})

			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("hotel_opportunities.id as total").first()
			...
			const data = await base
				...
				.select("hotel_opportunities.*", "hotel_opportunity_pivots.category", "hotel_opportunity_pivots.description", "hotels.location_id")
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("hotel_opportunities").whereNull("hotel_opportunities.deleted_at").select("hotel_opportunities.*", "hotel_opportunity_pivots.category", "hotel_opportunity_pivots.description").innerJoin("hotel_opportunity_pivots", "hotel_opportunities.id", "hotel_opportunity_pivots.hotel_opportunity_id").where("hotel_opportunity_pivots.language_code", language).whereNull("hotel_opportunity_pivots.deleted_at")
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { hotel_id, category, description } = req.body as {
				hotel_id: string
				category: string
				description: string
			}
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const updatedHotelOpportunity = await new HotelOpportunityModel().oneToMany(id, "hotel_opportunity_pivots", "hotel_opportunity_id")

			await new HotelModel().update(existingHotelOpportunity.hotel_id, { admin_approval: false, status: false })
			return res.status(200).send({
				success: true,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("hotel_opportunity_pivots").where("hotel_opportunity_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
