...
export default class HotelGalleryController {
			...
			const base = knex("hotel_galleries")
				...
				.modify(qb => {
					...
					if (search) {
						const like = `%${search}%`
						qb.andWhere(w => {
							w.where("hotel_galleries.image_type", "ilike", like).orWhere("hotel_gallery_pivot.category", "ilike", like)
						})
					}
				})

			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("hotel_galleries.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("hotel_galleries").whereNull("hotel_galleries.deleted_at").leftJoin("hotel_gallery_pivot", "hotel_galleries.id", "hotel_gallery_pivot.hotel_gallery_id").where("hotel_gallery_pivot.language_code", language).whereNull("hotel_gallery_pivot.deleted_at").select("hotel_galleries.*", "hotel_gallery_pivot.category")
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const image = await knex("hotel_galleries").whereNull("hotel_galleries.deleted_at").leftJoin("hotel_gallery_pivot", "hotel_galleries.id", "hotel_gallery_pivot.hotel_gallery_id").where("hotel_gallery_pivot.language_code", language).whereNull("hotel_gallery_pivot.deleted_at").where("hotel_galleries.id", id).select("hotel_galleries.*", "hotel_gallery_pivot.category").first()
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingHotelGallery = await new HotelGalleryModel().hasCoverImage(hotel_id)
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			if (category) {
				await knex("hotel_gallery_pivot").where({ hotel_gallery_id: id }).update({ deleted_at: new Date() })
			...
			await new HotelModel().update(updatedImage.hotel_id, { admin_approval: false, status: false })
			return res.status(200).send({
				success: true,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex.transaction(async trx => {
				...
				await trx("hotel_galleries").where({ id }).update({ deleted_at: new Date() })

				// Soft delete translations
				await trx("hotel_gallery_pivot").where({ hotel_gallery_id: id }).update({ deleted_at: new Date() })
			})
	...
	async bulkDelete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingImages = await knex("hotel_galleries").whereIn("id", ids).whereNull("deleted_at")
			...
			await knex.transaction(async trx => {
				...
				await trx("hotel_galleries").whereIn("id", ids).update({ deleted_at: new Date() })

				// Soft delete translations
				await trx("hotel_gallery_pivot").whereIn("hotel_gallery_id", ids).update({ deleted_at: new Date() })
			})
