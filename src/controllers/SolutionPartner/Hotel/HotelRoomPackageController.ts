...
export class HotelRoomPackageController {
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const packageModel = await knex
				.select("hotel_room_packages.*", knex.raw("COALESCE(json_agg(hotel_room_package_prices.*) FILTER (WHERE hotel_room_package_prices.id IS NOT NULL), '[]'::json) as prices"))
				.from("hotel_room_packages")
				.leftJoin("hotel_room_package_prices", "hotel_room_packages.id", "hotel_room_package_prices.hotel_room_package_id")
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { hotel_room_id, discount, total_tax_amount, constant_price, prices } = req.body as CreatePackageBody

			const existingHotelRoom = await new HotelRoomPackageModel().exists({
				hotel_room_id,
			})
			console.log(existingHotelRoom, "existingHotelRoom")
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Delete old prices
			await knex("hotel_room_package_prices").del().where("hotel_room_package_id", id).whereNull("deleted_at")
