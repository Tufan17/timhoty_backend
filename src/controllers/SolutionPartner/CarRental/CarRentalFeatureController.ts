...
export default class CarRentalFeatureController {
    ...
    try {
      ...
      const base = knex("car_rental_features")
        ...
        .innerJoin(
          ...
          "car_rental_features.id",
          "car_rental_feature_pivots.car_rental_feature_id"
        )
        .innerJoin(
          "car_rentals",
          "car_rental_features.car_rental_id",
          "car_rentals.id"
        )
        ...
        .modify((qb) => {
          ...
          if (car_rental_id)
            qb.where("car_rental_features.car_rental_id", car_rental_id);
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("car_rental_features.id as total")
        .first();
      ...
      const data = await base
        ...
        .select(
          "car_rental_features.*",
          "car_rental_feature_pivots.name",
  ...
  async findAll(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      let query = knex("car_rental_features")
        .whereNull("car_rental_features.deleted_at")
    ...
    try {
      ...
      if (existingFeature) {
        return res.status(400).send({
          success: false,
          message: req.t(
            "CAR_RENTAL_FEATURE.CAR_RENTAL_FEATURE_ALREADY_EXISTS"
          ),
        });
      }
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const updatedCarRentalFeature =
        await new CarRentalFeatureModel().oneToMany(
          id,
          "car_rental_feature_pivots",
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("car_rental_feature_pivots")
        .where("car_rental_feature_id", id)
