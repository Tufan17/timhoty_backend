...
export class CarRentalPackageController {
      ...
      const query = knex("car_rental_packages")
        ...
        .where(function () {
          if (search) {
            const like = `%${search}%`;
            this.where(function () {
              this.where("car_rental_packages.discount", "ilike", like).orWhere(
        ...
        .select(
          "car_rental_packages.*",
          knex.raw("json_agg(car_rental_package_prices.*) as prices")
        )
        .groupBy("car_rental_packages.id");

      const countResult = await knex("car_rental_packages")
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("car_rental_package_prices")
        .del()
        .where("car_rental_package_id", id)
        .whereNull("deleted_at");
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existingPackage = await knex("car_rental_packages")
        .where("id", id)
        .whereNull("deleted_at")
        .first();
      ...
      // Soft delete pivot
      await knex("car_rental_package_pivots")
        .where("car_rental_package_id", id)
        .update({ deleted_at: knex.fn.now() });
