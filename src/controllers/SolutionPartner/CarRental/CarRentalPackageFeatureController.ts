...
export default class CarRentalPackageFeatureController {
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const base = knex("car_rental_package_features")
        ...
        .innerJoin(
          "car_rental_packages",
          "car_rental_package_features.car_rental_package_id",
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("car_rental_package_features.id as total")
        .first();
  ...
  async create(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existingCarRentalPackage = await new CarRentalPackageModel().exists({
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existingFeature = await knex("car_rental_package_features")
        .where("id", id)
        .first();
      ...
      // Validate hotel room if hotel_room_id is provided
        if (car_rental_package_id) {
        const carRentalPackage = await knex("car_rental_packages")
          .where("id", car_rental_package_id)
          .first();
      ...
      const updatedFeature = await knex("car_rental_package_features")
        ...
        .where(
          "car_rental_package_feature_pivots.language_code",
          (req as any).language
        )
        .first();
  ...
  async delete(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      await knex("car_rental_package_feature_pivots")
        .where("car_rental_package_feature_id", id)
