...
export default class StationController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("stations")
				...
				.select(knex.raw("CONCAT(city_pivots.name, ', ', country_pivots.name) as address"))
				.modify(qb => {
					if (typeof status !== "undefined") qb.where("stations.status", status)
			...
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("stations.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			const { search, location_id } = req.query as { search: string; location_id?: string }
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let body: any = {
				location_id: location_id !== undefined ? location_id : existingStation.location_id,
				status: false,
				map_location: map_location !== undefined ? map_location : existingStation.map_location,
			}
			...
			const updatedStation = await new StationModel().oneToMany(id, "station_pivots", "station_id")
