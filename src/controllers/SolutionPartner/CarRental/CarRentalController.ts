...
export default class CarRentalController {
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const base = knex("car_rentals")
        ...
        .modify((qb) => {
          ...
          if (spFromUser) qb.where("car_rentals.solution_partner_id", spFromUser);
            ...
            qb.where("car_rentals.solution_partner_id", solution_partner_id);

          if (typeof status !== "undefined") qb.where("car_rentals.status", status);
          if (typeof admin_approval !== "undefined")
            qb.where("car_rentals.admin_approval", admin_approval);
          if (typeof highlight !== "undefined")
            qb.where("car_rentals.highlight", highlight);
          ...
          if (search) {
            ...
            qb.andWhere((w) => {
              w.where("car_rental_pivots.title", "ilike", like)
                .orWhere("car_rental_pivots.general_info", "ilike", like)
                .orWhere("car_rental_pivots.car_info", "ilike", like)
                .orWhere("country_pivots.name", "ilike", like)
                .orWhere("city_pivots.name", "ilike", like)
                .orWhere("car_type_pivots.name", "ilike", like)
                .orWhere("gear_type_pivots.name", "ilike", like);
            }); 
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("car_rentals.id as total")
        .first();
      ...
      const data = await base
        ...
        .select(
          "car_rentals.*",
          "car_rental_pivots.title as title",
          ...
          knex.ref("car_type_pivots.name").as("car_type_name"),
          knex.ref("gear_type_pivots.name").as("gear_type_name")
        )
  ...
  async findOne(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      if (carRental.car_type_id) {
        const carType = await knex("car_type_pivots")
          .where("car_type_pivots.car_type_id", carRental.car_type_id)
          .where("car_type_pivots.language_code", req.language)
          .whereNull("car_type_pivots.deleted_at")
          .select("car_type_pivots.name")
          .first();
        carRental.car_type_name = carType?.name;
      }

      if (carRental.gear_type_id) {
        const gearType = await knex("gear_type_pivots")
          .where("gear_type_pivots.gear_type_id", carRental.gear_type_id)
          .where("gear_type_pivots.language_code", req.language)
          .whereNull("gear_type_pivots.deleted_at")
          .select("gear_type_pivots.name")
          .first();
      ...
      const carRentalPackages = await knex("car_rental_packages")
        ...
        .select("car_rental_packages.*", "car_rental_package_pivots.name");
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      let body: any = {
        location_id:
          location_id !== undefined ? location_id : existingCarRental.location_id,
        solution_partner_id:
          solution_partner_id !== undefined
            ? solution_partner_id
            : existingCarRental.solution_partner_id,
      ...
      const updatedCarRental = await new CarRentalModel().oneToMany(
        id,
        "car_rental_pivots",
        "car_rental_id"
      );
