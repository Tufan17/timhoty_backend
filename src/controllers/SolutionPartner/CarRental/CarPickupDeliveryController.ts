...
export default class CarPickupDeliveryController {
  async dataTable(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const countRow = await base
        ...
        .clearSelect()
        .clearOrder()
        .countDistinct<{ total: string }>("car_pickup_delivery.id as total")
        .first();
      ...
      const data = await base
        ...
        .select(
          "car_pickup_delivery.*",
          "car_rentals.title as car_rental_title",
  ...
  async findOne(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const pickupDelivery = await knex("car_pickup_delivery")
        .whereNull("car_pickup_delivery.deleted_at")
  ...
  async create(req: FastifyRequest, res: FastifyReply) {
    try {
      const solutionPartnerUser = (req as any).user;
      const { car_rental_id, station_id, status = false } = req.body as {
        car_rental_id: string;
        station_id: string;
        status?: boolean;
      };
