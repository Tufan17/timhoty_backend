...
export default class SolutionPartnerUserController {
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			return res.status(200).send({
				success: true,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_FETCHED_SUCCESS"),
				data: data,
			...
			return res.status(500).send({
				success: false,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_FETCHED_ERROR"),
			})
		}
	}
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const solution_partner_users = await new SolutionPartnerUserModel().first({ id })
			return res.status(200).send({
				success: true,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_FETCHED_SUCCESS"),
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { solution_partner_id, type, name_surname, phone, email, password, language_code, status } = req.body as {
				solution_partner_id: string
				type: string
				name_surname: string
				phone: string
			...
			const existingSolutionPartnerUser = await new SolutionPartnerUserModel().first({ email })

			if (existingSolutionPartnerUser) {
				return res.status(400).send({
					success: false,
					message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_ALREADY_EXISTS"),
				})
			}
			...
			return res.status(200).send({
				success: true,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_CREATED_SUCCESS"),
			...
			return res.status(500).send({
				success: false,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_CREATED_ERROR"),
			})
		}
	}
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			if (!existingSolutionPartnerUser) {
				return res.status(404).send({
					success: false,
					message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_NOT_FOUND"),
				})
			}
			...
			if (email && email !== existingSolutionPartnerUser.email) {
				...
				const existingSolutionPartnerUserByEmail = await new SolutionPartnerUserModel().first({ email })
			...
			return res.status(200).send({
				success: true,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_UPDATED_SUCCESS"),
			...
			return res.status(500).send({
				success: false,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_UPDATED_ERROR"),
			})
		}
	}
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingSolutionPartnerUser = await new SolutionPartnerUserModel().first({
				id,
			})
			...
			return res.status(200).send({
				success: true,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_DELETED_SUCCESS"),
			})
		} catch (error) {
			console.log(error)
			return res.status(500).send({
				success: false,
				message: req.t("SOLUTION_PARTNER_USER.SOLUTION_PARTNER_USER_DELETED_ERROR"),
			})
		}
	}
}
