...
export default class TourPackageOpportunityController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("tour_package_opportunities.id as total").first()
			...
			const data = await base
				...
				.select("tour_package_opportunities.*", "tour_package_opportunity_pivots.name", "tour_packages.location_id")
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { tour_package_id, name } = req.body as {
				tour_package_id: string
				name: string
			}
			...
			await new TourModel().update(existTourPackage.tour_id, {
				status: false,
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const updatedTourPackageOpportunity = await new TourPackageOpportunityModel().oneToMany(id, "tour_package_opportunity_pivots", "tour_package_opportunity_id")
			const existingTourPackage = await new TourPackageModel().first({
				"tour_packages.id": tour_package_id,
			})
			if (!existingTourPackage) {
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("tour_package_opportunity_pivots").where("tour_package_opportunity_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
