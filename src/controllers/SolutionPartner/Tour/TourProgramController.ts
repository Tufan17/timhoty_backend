...
export default class TourProgramController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("tour_programs.id as total").first()
			...
			const data = await base
				...
				.select("tour_programs.*", "tour_program_pivots.name", "tours.location_id")
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("tour_programs").whereNull("tour_programs.deleted_at").select("tour_programs.*", "tour_program_pivots.name").innerJoin("tour_program_pivots", "tour_programs.id", "tour_program_pivots.tour_program_id").where("tour_program_pivots.language_code", language).whereNull("tour_program_pivots.deleted_at")
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const tourProgram = await knex("tour_programs").whereNull("tour_programs.deleted_at").where("tour_programs.id", id).select("tour_programs.*", "tour_program_pivots.name").innerJoin("tour_program_pivots", "tour_programs.id", "tour_program_pivots.tour_program_id").where("tour_program_pivots.language_code", language).whereNull("tour_program_pivots.deleted_at").first()
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await new TourModel().update(tourProgram.tour_id, {
				status: false,
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const updatedTourProgram = await new TourProgramModel().oneToMany(id, "tour_program_pivots", "tour_program_id")
			await new TourModel().update(existingTourProgram.tour_id, {
				status: false,
				admin_approval: false,
			})
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("tour_program_pivots").where("tour_program_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
