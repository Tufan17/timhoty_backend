...
export class TourPackageController {
			...
			const query = knex("tour_packages")
				...
				.where(function () {
					if (search) {
						const like = `%${search}%`
						this.where(function () {
							this.where("tour_package_prices.discount", "ilike", like).orWhere("tour_package_prices.total_tax_amount", "ilike", like)

							if (search.toLowerCase() === "true" || search.toLowerCase() === "false") {
								this.orWhere("tour_packages.constant_price", search.toLowerCase() === "true")
							}
						})
					}
				})
				.select("tour_packages.*", knex.raw("json_agg(tour_package_prices.*) as prices"))
				.groupBy("tour_packages.id")

			const countResult = await knex("tour_packages").whereNull("deleted_at").where("tour_id", tour_id).count("* as total").first()
		...
		try {
          ...
          `),
				...
				.leftJoin("currency_pivots", function () {
					this.on("tour_package_prices.currency_id", "currency_pivots.currency_id").andOn("currency_pivots.language_code", knex.raw("?", [req.language]))
				})
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { tour_id, name, description, refund_policy, return_acceptance_period } = req.body as CreatePackageBody
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const { return_acceptance_period, refund_policy, name, description } = req.body as UpdatePackageBody

			const packageModel = await knex.select("tour_packages.*", knex.raw("json_agg(tour_package_prices.*) as prices")).from("tour_packages").leftJoin("tour_package_prices", "tour_packages.id", "tour_package_prices.tour_package_id").where("tour_packages.id", id).whereNull("tour_package_prices.deleted_at").whereNull("tour_packages.deleted_at").groupBy("tour_packages.id").first()
			...
			const updatedPackage = await knex.select("tour_packages.*", knex.raw("json_agg(tour_package_prices.*) as prices")).from("tour_packages").leftJoin("tour_package_prices", "tour_packages.id", "tour_package_prices.tour_package_id").where("tour_packages.id", id).whereNull("tour_package_prices.deleted_at").whereNull("tour_packages.deleted_at").groupBy("tour_packages.id").first()

			await new TourModel().update(packageModel.tour_id, {
				status: false,
				admin_approval: false,
			})
			return res.status(200).send({
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingPackage = await knex("tour_packages").where("id", id).whereNull("deleted_at").first()
			...
			// Soft delete package
			await knex("tour_packages").where("id", id).update({ deleted_at: knex.fn.now() })

			// Soft delete pivot
			await knex("tour_package_pivots").where("tour_package_id", id).update({ deleted_at: knex.fn.now() })

			// Soft delete prices
			await knex("tour_package_prices").where("tour_package_id", id).update({ deleted_at: knex.fn.now() })
