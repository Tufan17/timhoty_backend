...
export default class TourPackagePriceController {
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { tour_package_id, main_price, child_price, baby_price, currency_id, period, date, quota, discount, total_tax_amount, single } = req.body as {
			...
			if (date) {
				const existingPrice = await knex("tour_package_prices").where("tour_package_id", tour_package_id).where("date", date).whereNull("deleted_at").first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const { main_price, child_price, baby_price, currency_id, period, date, quota, discount, total_tax_amount, single } = req.body as {
				main_price?: number
				child_price?: number
				baby_price?: number
				currency_id?: string
			...
			if (date) {
				const duplicatePrice = await knex("tour_package_prices").where("tour_package_id", existingPrice.tour_package_id).where("date", date).whereNot("id", id).whereNull("deleted_at").first()
			...
			const tourPackage = await new TourPackageModel().first({
				id: updatedPrice.tour_package_id,
			})
			if (!tourPackage) {
				return res.status(400).send({
					success: false,
