...
export default class TourFeatureController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("tour_features.id as total").first()
			...
			const data = await base
				...
				.select("tour_features.*", "tour_feature_pivots.name", "tours.location_id")
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("tour_features").whereNull("tour_features.deleted_at").select("tour_features.*", "tour_feature_pivots.name").innerJoin("tour_feature_pivots", "tour_features.id", "tour_feature_pivots.tour_feature_id").where("tour_feature_pivots.language_code", language).whereNull("tour_feature_pivots.deleted_at")
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const tourFeature = await knex("tour_features").whereNull("tour_features.deleted_at").where("tour_features.id", id).select("tour_features.*", "tour_feature_pivots.name").innerJoin("tour_feature_pivots", "tour_features.id", "tour_feature_pivots.tour_feature_id").where("tour_feature_pivots.language_code", language).whereNull("tour_feature_pivots.deleted_at").first()
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const updatedTourFeature = await new TourFeatureModel().oneToMany(id, "tour_feature_pivots", "tour_feature_id")
			await new TourModel().update(existingTourFeature.tour_id, {
				status: false,
				admin_approval: false,
			})
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("tour_feature_pivots").where("tour_feature_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
