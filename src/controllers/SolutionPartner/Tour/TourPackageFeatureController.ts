...
export default class TourPackageFeatureController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const base = knex("tour_package_features")
				...
				.innerJoin("tour_packages", "tour_package_features.tour_package_id", "tour_packages.id")
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("tour_package_features.id as total").first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("tour_package_features").whereNull("tour_package_features.deleted_at").select("tour_package_features.*", "tour_package_feature_pivots.name").innerJoin("tour_package_feature_pivots", "tour_package_features.id", "tour_package_feature_pivots.tour_package_feature_id").where("tour_package_feature_pivots.language_code", language).whereNull("tour_package_feature_pivots.deleted_at")

			if (tour_package_id) {
				query = query.where("tour_package_features.tour_package_id", tour_package_id)
			}
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await new TourModel().update(existingTourPackage.tour_id, {
				status: false,
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const existingFeature = await knex("tour_package_features").where("id", id).first()
			...
			if (tour_package_id) {
				const tourPackage = await knex("tour_packages").where("id", tour_package_id).first()
			...
			const updatedFeature = await knex("tour_package_features")
				.where("tour_package_features.id", id)
				.select("tour_package_features.*", "tour_package_feature_pivots.name")
				.leftJoin("tour_package_feature_pivots", "tour_package_features.id", "tour_package_feature_pivots.tour_package_feature_id")
				.where("tour_package_feature_pivots.language_code", (req as any).language)
				.first()
			const tourPackage = await new TourPackageModel().first({
				id: updatedFeature.tour_package_id,
			})
			if (!tourPackage) {
				return res.status(400).send({
					success: false,
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("tour_package_feature_pivots").where("tour_package_feature_id", id).update({ deleted_at: new Date() })
			return res.status(200).send({
				success: true,
