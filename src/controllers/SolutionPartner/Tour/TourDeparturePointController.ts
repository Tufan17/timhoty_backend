...
export default class TourDeparturePointController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Count total records
			const countRow = await base.clone().clearSelect().clearOrder().countDistinct<{ total: string }>("tour_departure_points.id as total").first()
			...
			const data = await base
				...
				.select("tour_departure_points.*", "city_pivots.name as city_name", "tours.location_id")
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			let query = knex("tour_departure_points").whereNull("tour_departure_points.deleted_at").select("tour_departure_points.*", "city_pivots.name as city_name").innerJoin("cities", "tour_departure_points.location_id", "cities.id").innerJoin("city_pivots", "cities.id", "city_pivots.city_id").where("city_pivots.language_code", language).whereNull("city_pivots.deleted_at")
