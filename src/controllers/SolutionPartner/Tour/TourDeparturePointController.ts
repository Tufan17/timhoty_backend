...
export default class TourDeparturePointController {
  ...
  async create(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existCity = await new CityModel().findId(location_id);
      if (!existCity) {
        return res.status(400).send({
          success: false,
      ...
      // Check if tour departure point already exists
      const existingTourDeparturePoint = await new TourDeparturePointModel().first({
        tour_id,
        location_id,
      });

      if (existingTourDeparturePoint) {
        return res.status(400).send({
          success: false,
  ...
  async update(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existingTourDeparturePoint = await new TourDeparturePointModel().first({ id });
      ...
      // Update tour departure point
      await new TourDeparturePointModel().update(id, {
        ...(tour_id && { tour_id }),
        ...(location_id && { location_id }),
      });

      const updatedTourDeparturePoint = await new TourDeparturePointModel().first({ id });

      return res.status(200).send({
        success: true,
        message: req.t("TOUR_DEPARTURE_POINT.TOUR_DEPARTURE_POINT_UPDATED_SUCCESS"),
        data: updatedTourDeparturePoint,
      });
    } catch (error) {
      console.log(error);
      return res.status(500).send({
