...
export default class StationController {
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			// Status değerinin boolean olduğunu kontrol et
			if (typeof status !== "boolean") {
				return res.status(400).send({
					success: false,
					message: req.t("STATION.STATION_STATUS_BOOLEAN_REQUIRED"),
				})
			}

			// Station'ın var olduğunu kontrol et
			...
			await new StationModel().update(id, { status })
