...
export default class SupportDealerController {
  async create(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const { title, description, docs,dealer_id } = fields;
  ...
  async findAll(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const baseQuery = knex("support_dealer")
        .leftJoin("dealer_users", "support_dealer.dealer_user_id", "dealer_users.id")
        .leftJoin("dealers", "support_dealer.dealer_id", "dealers.id");
  
      // Ana data sorgusu
      const query = baseQuery.clone().where((builder) => {
        // Arama koşulları gruplanmalı
        if (search) {
          builder.andWhere((qb) => {
            qb.where("support_dealer.title", "ilike", `%${search}%`)
              .orWhere("support_dealer.description", "ilike", `%${search}%`)
              .orWhere("dealers.name", "ilike", `%${search}%`);
          });
        }
  
        if (start_date) builder.andWhere("support_dealer.created_at", ">=", start_date);
        if (end_date) builder.andWhere("support_dealer.created_at", "<=", end_date);
        if (status) builder.andWhere("support_dealer.status", status);
        if (dealer_id) builder.andWhere("support_dealer.dealer_id", dealer_id);
      });
  
      const countQuery = baseQuery.clone().where((builder) => {
        builder.andWhere("support_dealer.dealer_user_id", user.id);
  ...
  async updateMessage(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      const existingMessage = await new SupportDealerModel().findId(id);

      if (!existingMessage) {
        return res.status(400).send({
          success: false,
          message: "Destek talebi mesajı bulunamadı",
        });
      }

      if (existingMessage.status === "waiting") {
        await new SupportDealerModel().update(existingMessage.id, {
          status: "pending",
        });
      }
      ...
      await new UserNotification().create({
        ...
        target_id: existingMessage.dealer_user_id,
        link: `/support/${id}`,
        target_type: "dealer",
      });
