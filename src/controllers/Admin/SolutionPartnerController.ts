...
export default class SolutionPartnerController {
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			const {
				...
				limit = 10,
				search = "",
				admin_verified = undefined,
				status = undefined,
			} = req.query as {
				page: number
				limit: number
				search: string
				admin_verified: string | undefined
			...
			const query = knex("solution_partners")
				...
				.where(function () {
					this.where("name", "ilike", `%${search}%`).orWhere("phone", "ilike", `%${search}%`).orWhere("address", "ilike", `%${search}%`).orWhere("tax_office", "ilike", `%${search}%`).orWhere("tax_number", "ilike", `%${search}%`).orWhere("bank_name", "ilike", `%${search}%`).orWhere("swift_number", "ilike", `%${search}%`).orWhere("account_owner", "ilike", `%${search}%`).orWhere("iban", "ilike", `%${search}%`)
				...
				.modify(queryBuilder => {
					if (admin_verified !== undefined) {
						queryBuilder.where("admin_verified", admin_verified === "true")
					}
					if (status !== undefined) {
						queryBuilder.where("status", status === "true")
					}
				})
	...
	async create(req: FastifyRequest, res: FastifyReply) {
		try {
			const { name, phone, address, tax_office, tax_number, bank_name, swift_number, account_owner, iban, language_code, location_id } = req.body as {
				name: string
				phone: string
				address: string
				tax_office: string
	...
	async update(req: FastifyRequest, res: FastifyReply) {
		try {
			const { id } = req.params as { id: string }
			const { name, phone, address, tax_office, tax_number, bank_name, swift_number, account_owner, iban, language_code, location_id, status, admin_verified } = req.body as {
				name: string
				phone: string
				address: string
				tax_office: string
