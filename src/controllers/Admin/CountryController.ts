...
export default class CountryController {
	async dataTable(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const countResult = await query
				...
				.clearSelect()
				.clearOrder()
				.clearGroup() // ← groupBy'ı temizle!
				.countDistinct("countries.id as total")
				.first()
	...
	async findAll(req: FastifyRequest, res: FastifyReply) {
		try {
			const language = req.language
			const countries = await knex("countries").whereNull("countries.deleted_at").select("countries.*", "country_pivots.name as name").innerJoin("country_pivots", "countries.id", "country_pivots.country_id").where("country_pivots.language_code", language)
			return res.status(200).send({
				success: true,
				message: req.t("COUNTRY.COUNTRY_FETCHED_SUCCESS"),
	...
	async findOne(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const country = await knex("countries").where("countries.id", id).whereNull("countries.deleted_at").innerJoin("country_pivots", "countries.id", "country_pivots.country_id").where("country_pivots.language_code", req.language).select("countries.*", "country_pivots.name as name").first()
			...
			const currency = await knex("currencies").where("currencies.id", country.currency_id).leftJoin("currency_pivots", "currencies.id", "currency_pivots.currency_id").where("currency_pivots.language_code", req.language).whereNull("currencies.deleted_at").first()
			if (currency) {
	...
	async delete(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			await knex("cities").where("country_id", id).whereNull("deleted_at").update({ deleted_at: new Date() })
			return res.status(200).send({
				success: true,
