			...
			if (charge.status === "CAPTURED") {
				...
				if (updatedReservation && updatedReservation.status === true) {
					...
					if (user) {
						...
						if (reservationDetails && reservationDetails.visa_id) {
...
async function sendPartnerNotificationEmail(reservationDetails: any, customer: any) {
	try {
		...
		const visa = await new VisaModel().first({ id: reservationDetails.visa_id })

		if (!visa || !visa.solution_partner_id) {
			console.log("Visa has no solution partner")
			return
		}

		// Solution partner'ın manager kullanıcısını bul
		const managerUser = await new SolutionPartnerUserModel().first({
			solution_partner_id: visa.solution_partner_id,
			type: "manager"
		})
		...
		html = html.replace(/\{\{title\}\}/g, reservationDetails.visa_title || "")
		html = html.replace(/\{\{city\}\}/g, reservationDetails.visa_city || "")
		html = html.replace(/\{\{country\}\}/g, reservationDetails.visa_country || "")
		if (reservationDetails.visa_city && reservationDetails.visa_city.trim() !== "") {
		...
		// Saat bilgisi varsa
		if (reservationDetails.visa_hour) {
			const minute = reservationDetails.visa_minute || "00"
			html = html.replace(/\{\{hour\}\}/g, reservationDetails.visa_hour.toString())
			html = html.replace(/\{\{minute\}\}/g, minute.toString())
		} else {
			html = html.replace(/<!-- \{\{HOUR_SECTION_START\}\} -->[\s\S]*?<!-- \{\{HOUR_SECTION_END\}\} -->/g, "")
		}
		...
		if (reservationDetails.visa_city) {
			html = html.replace(/<!-- \{\{COUNTRY_ONLY_SECTION_START\}\} -->[\s\S]*?<!-- \{\{COUNTRY_ONLY_SECTION_END\}\} -->/g, "")
		} else {
			html = html.replace(/<!-- \{\{CITY_SECTION_START\}\} -->[\s\S]*?<!-- \{\{CITY_SECTION_END\}\} -->/g, "")
			html = html.replace(/<!-- \{\{LOCATION_SECTION_START\}\} -->[\s\S]*?<!-- \{\{LOCATION_SECTION_END\}\} -->/g, "")
