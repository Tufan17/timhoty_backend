...
class UserVisaPayment {
	 ...
	 */
	async createPaymentIntent(req: FastifyRequest<{ Body: CreateCarRentalPaymentRequest }>, res: FastifyReply) {
		try {
			const { amount, currency = "USD", customer, car_rental_id, booking_id, description, start_date, end_date, users, different_invoice, package_id } = req.body
