  ...
  async index(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      if (tours.length > 0) {
        ...
        const departurePointsByTourId = allDeparturePoints.reduce(
        ...
        const locationsByTourId = allLocations.reduce(
        ...
        const allTourPackages = await knex("tour_packages")
          ...
          .innerJoin("tour_package_pivots", function () {
            this.on(
              ...
              "tour_package_pivots.language_code",
              knex.raw("?", [language])
            );
          })
        ...
        if (allTourPackages.length > 0) {
          ...
          allTourPackages.forEach((pkg: any) => {
            const prices = allPackagePrices.filter(
              (price: any) => price.tour_package_id === pkg.id
            );
          ...
          tours.forEach((tour: any) => {
            const tourPackages = allTourPackages.filter(
              (pkg: any) => pkg.tour_id === tour.id
            );
      ...
      if (isAvailable) {
        filteredTours = filteredTours.filter(
          (tour: any) => tour.total_price > 0
        );
      }

      if (min_price) {
        filteredTours = filteredTours.filter(
          (tour: any) => tour.total_price >= Number(min_price)
        );
      }

      if (max_price) {
        filteredTours = filteredTours.filter(
          (tour: any) => tour.total_price <= Number(max_price)
        );
      }
    ...
    try {
      ...
      const results = await knex("tours")
        ...
        .leftJoin("tour_locations", function () {
          this.on("tours.id", "tour_locations.tour_id").andOnNull(
            "tour_locations.deleted_at"
          );
        })
        .leftJoin(
          "cities as location_cities",
          "tour_locations.location_id",
          "location_cities.id"
        )
        .leftJoin("city_pivots as location_city_pivots", function () {
          this.on("location_cities.id", "location_city_pivots.city_id").andOn(
            "location_city_pivots.language_code",
            knex.raw("?", [language])
          );
        })
        .leftJoin("country_pivots as location_country_pivots", function () {
          this.on(
            ...
            "location_country_pivots.country_id"
          ).andOn(
            "location_country_pivots.language_code",
            knex.raw("?", [language])
          );
        })
        ...
        .leftJoin("tour_departure_points", function () {
          this.on("tours.id", "tour_departure_points.tour_id").andOnNull(
            "tour_departure_points.deleted_at"
          );
        })
        .leftJoin(
          "cities as departure_cities",
          "tour_departure_points.location_id",
          "departure_cities.id"
        )
        .leftJoin("city_pivots as departure_city_pivots", function () {
          this.on("departure_cities.id", "departure_city_pivots.city_id").andOn(
            "departure_city_pivots.language_code",
            knex.raw("?", [language])
          );
        })
        .leftJoin("country_pivots as departure_country_pivots", function () {
          this.on(
            "departure_cities.country_id",
            "departure_country_pivots.country_id"
          ).andOn(
            "departure_country_pivots.language_code",
            knex.raw("?", [language])
          );
        })
        ...
        .leftJoin("tour_galleries", function () {
          this.on("tours.id", "tour_galleries.tour_id").andOnNull(
            "tour_galleries.deleted_at"
          );
        })
        .leftJoin("tour_gallery_pivots", function () {
          this.on("tour_galleries.id", "tour_gallery_pivots.tour_gallery_id")
            .andOn(
              "tour_gallery_pivots.language_code",
              knex.raw("?", [language])
            )
        ...
        .leftJoin("tour_features", function () {
          this.on("tours.id", "tour_features.tour_id").andOnNull(
            "tour_features.deleted_at"
          );
        })
        .leftJoin("tour_feature_pivots", function () {
          this.on("tour_features.id", "tour_feature_pivots.tour_feature_id")
            .andOn(
              "tour_feature_pivots.language_code",
              knex.raw("?", [language])
            )
        ...
        .leftJoin("tour_packages", function () {
          this.on("tours.id", "tour_packages.tour_id").andOnNull(
            "tour_packages.deleted_at"
          );
        })
        ...
        .leftJoin("tour_package_images", function () {
          this.on(
            "tour_packages.id",
            "tour_package_images.tour_package_id"
          ).andOnNull("tour_package_images.deleted_at");
        })
        ...
        .leftJoin("tour_package_opportunities", function () {
          this.on(
            "tour_packages.id",
            "tour_package_opportunities.tour_package_id"
        ...
        .leftJoin("tour_programs", function () {
          this.on("tours.id", "tour_programs.tour_id").andOnNull(
            "tour_programs.deleted_at"
          );
        })
        .leftJoin("tour_program_pivots", function () {
          this.on("tour_programs.id", "tour_program_pivots.tour_program_id")
            .andOn(
              "tour_program_pivots.language_code",
              knex.raw("?", [language])
            )
      ...
      results.forEach((row: any) => {
        ...
        if (row.package_image_id) {
          const existingImage = packageData.images.find(
            (img: any) => img.id === row.package_image_id
          );
          if (!existingImage) {
        ...
        if (row.package_opportunity_id) {
          const existingOpportunity = packageData.opportunities.find(
            (opp: any) => opp.id === row.package_opportunity_id
          );
        ...
        if (row.package_feature_id) {
          const existingFeature = packageData.features.find(
            (feat: any) => feat.id === row.package_feature_id
          );
          if (!existingFeature) {
        ...
        if (row.price_id) {
          const existingPrice = packageData.prices.find(
            (p: any) => p.id === row.price_id
          );
          if (!existingPrice) {
      ...
      tour.departure_points = Array.from(
        tourDeparturePointsMap.values()
      ) as any;
