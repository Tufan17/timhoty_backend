...
export default class car_rentalController {
	async index(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const { location_id, station_id, page = 1, limit = 5, guest_rating, arrangement, isAvailable, min_price, max_price, type, gear_type, user_count, door_count, air_conditioning, pickup_date, dropoff_date, age } = req.query as any
				...
				.modify(function (queryBuilder) {
					if (station_id) {
						queryBuilder.whereIn("car_rentals.id", function () {
							this.select("car_rental_id").from("car_pickup_delivery").where("station_id", station_id).whereNull("deleted_at")
						})
					}
				})
			...
			const countQuery = knex("car_rentals")
				...
				.modify(function (queryBuilder) {
					...
					if (age) {
						queryBuilder.where("car_rentals.age_limit", ">=", age)
					}
				})
				.countDistinct("car_rentals.id as total")
