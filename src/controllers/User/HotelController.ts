...
export default class HotelController {
  ...
  async show(req: FastifyRequest, res: FastifyReply) {
    ...
    const results = await knex("hotels")
      ...
      .leftJoin("hotel_galleries", "hotels.id", "hotel_galleries.hotel_id")
      .leftJoin("hotel_gallery_pivot", function () {
        this.on("hotel_galleries.id", "hotel_gallery_pivot.hotel_gallery_id")
          ...
          .andOnNull("hotel_galleries.deleted_at")
          .andOnNull("hotel_gallery_pivot.deleted_at");
      })

      // Otel olanakları
      .leftJoin("hotel_opportunities", "hotels.id", "hotel_opportunities.hotel_id")
      .leftJoin("hotel_opportunity_pivots", function () {
        this.on("hotel_opportunities.id", "hotel_opportunity_pivots.hotel_opportunity_id")
          .andOn("hotel_opportunity_pivots.language_code", knex.raw("?", [language]))
          .andOnNull("hotel_opportunities.deleted_at")
      ...
      .leftJoin("hotel_feature_pivots", function () {
        this.on("hotel_features.id", "hotel_feature_pivots.hotel_feature_id")
          ...
          .andOnNull("hotel_features.deleted_at")
      ...
      .leftJoin("hotel_room_images", function () {
        this.on("hotel_rooms.id", "hotel_room_images.hotel_room_id")
          .andOnNull("hotel_room_images.deleted_at");
      })
      // hotel room opportunity
      .leftJoin("hotel_room_opportunities", "hotel_rooms.id", "hotel_room_opportunities.hotel_room_id")
      ...
      // hotel room feature
      .leftJoin("hotel_room_features", "hotel_rooms.id", "hotel_room_features.hotel_room_id")
      ...
      .leftJoin("currencies as currency_deleted", function () {
        this.on("hotel_room_package_prices.currency_id", "currency_deleted.id")
          .andOnNull("currency_deleted.deleted_at");
      })
    ...
    results.forEach((row: any) => {
      if (!row.gallery_image_url) return; // Gallery yoksa atla

      if (!galleryMap.has(row.gallery_image_url)) {
        galleryMap.set(row.gallery_image_url, {
