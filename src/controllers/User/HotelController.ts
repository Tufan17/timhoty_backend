...
async function calculatePriceForDateRange(packageId: string, checkInDate: Date, checkOutDate: Date): Promise<number> {
	...
	const pricePeriods = await knex("hotel_room_package_prices").where("hotel_room_package_id", packageId).whereNull("deleted_at").orderBy("start_date", "asc")
...
export default class HotelController {
	async index(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const { location_id, star_rating, page = 1, limit = 3, start_date, guest_rating, end_date, adult, child, arrangement, isAvailable, min_price, max_price } = req.query as any
			...
			let hotel = await knex("hotels")
				...
				.innerJoin("hotel_pivots", function () {
					this.on("hotels.id", "hotel_pivots.hotel_id").andOn("hotel_pivots.language_code", knex.raw("?", [language]))
				})
              ...
              LIMIT 1
				...
				.limit(limit)
				.offset((page - 1) * limit)
				.select("hotels.id", "hotel_pivots.name", "country_pivots.name as country_name", "city_pivots.name as city_name", "city_pivots.city_id as location_id", "country_pivots.country_id as country_id", "hotels.star_rating", "hotels.average_rating", "hotels.comment_count", "hotels.refund_days", "hotels.pool", "hotels.private_beach", "hotels.transfer", "hotels.map_location", "hotels.free_age_limit", "hotel_gallery.image_url")
			...
			const hotelRooms = await knex("hotel_rooms").whereIn("hotel_rooms.hotel_id", hotelIds).innerJoin("hotel_room_pivots", "hotel_rooms.id", "hotel_room_pivots.hotel_room_id").where("hotel_room_pivots.language_code", language).whereNull("hotel_rooms.deleted_at").select("hotel_rooms.refund_days", "hotel_rooms.id", "hotel_rooms.hotel_id", "hotel_room_pivots.name")
			...
			roomPackages.forEach(async (item: any) => {
				if (start_date && end_date) {
					...
					for (const day of days) {
						...
						const price = item.constant_price ? roomPackagePrices.find((room: any) => room.hotel_room_package_id === item.id) : roomPackagePrices.find((room: any) => room.hotel_room_package_id === item.id && room.start_date <= day && room.end_date >= day)
						if (price) {
							item.price = {
					...
					const price = roomPackagePrices.find((room: any) => room.hotel_room_package_id === item.id)
					item.price = {
						main_price: price.main_price,
						child_price: price.child_price,
					}
					...
					const now = new Date()
					const price = roomPackagePrices.find((room: any) => room.hotel_room_package_id === item.id && room.start_date <= now && room.end_date >= now)
					item.price = {
						main_price: price.main_price,
						child_price: price.child_price,
					}
			...
			hotelRooms.forEach(async (item: any) => {
				item.roomPackage = roomPackages.find((room: any) => room.hotel_room_id === item.id)
			})

			hotel.forEach((item: any) => {
				// Otele ait odaları bul
				const relatedRooms = hotelRooms.filter((room: any) => room.hotel_id === item.id)

				// En düşük fiyatlı odayı seç
				const cheapestRoom = relatedRooms.reduce((lowest: any, current: any) => {
		...
		try {
			...
			const results = await knex("hotels")
				...
				.leftJoin("hotel_rooms", function () {
					this.on("hotels.id", "hotel_rooms.hotel_id").andOnNull("hotel_rooms.deleted_at")
				})
				...
				.leftJoin("hotel_galleries", function () {
					this.on("hotels.id", "hotel_galleries.hotel_id").andOnNull("hotel_galleries.deleted_at")
				})
				.leftJoin("hotel_gallery_pivot", function () {
					this.on("hotel_galleries.id", "hotel_gallery_pivot.hotel_gallery_id")
						.andOn("hotel_gallery_pivot.language_code", knex.raw("?", [language]))
						.andOnNull("hotel_gallery_pivot.deleted_at")
				})

				// Otel olanakları
				.leftJoin("hotel_opportunities", function () {
					this.on("hotels.id", "hotel_opportunities.hotel_id").andOnNull("hotel_opportunities.deleted_at")
				})
				.leftJoin("hotel_opportunity_pivots", function () {
					this.on("hotel_opportunities.id", "hotel_opportunity_pivots.hotel_opportunity_id")
						.andOn("hotel_opportunity_pivots.language_code", knex.raw("?", [language])) // Düzeltildi
						.andOnNull("hotel_opportunity_pivots.deleted_at")
				})

				// Otel özellikleri
				.leftJoin("hotel_features", function () {
					this.on("hotels.id", "hotel_features.hotel_id").andOnNull("hotel_features.deleted_at")
				})
				.leftJoin("hotel_feature_pivots", function () {
					this.on("hotel_features.id", "hotel_feature_pivots.hotel_feature_id")
						.andOn("hotel_feature_pivots.language_code", knex.raw("?", [language]))
				...
				.leftJoin("hotel_room_packages", function () {
					this.on("hotel_rooms.id", "hotel_room_packages.hotel_room_id").andOnNull("hotel_room_packages.deleted_at")
				})
				.leftJoin("hotel_room_package_prices", function () {
					this.on("hotel_room_packages.id", "hotel_room_package_prices.hotel_room_package_id").andOnNull("hotel_room_package_prices.deleted_at")
				})

				// Otel oda resimleri (pivot tablosu yok, direkt image tablosu)
				.leftJoin("hotel_room_images", function () {
					this.on("hotel_rooms.id", "hotel_room_images.hotel_room_id").andOnNull("hotel_room_images.deleted_at")
				})
				// hotel room opportunity
				.leftJoin("hotel_room_opportunities", function () {
					this.on("hotel_rooms.id", "hotel_room_opportunities.hotel_room_id").andOnNull("hotel_room_opportunities.deleted_at")
				})
				.leftJoin("hotel_room_opportunity_pivots", function () {
					this.on("hotel_room_opportunities.id", "hotel_room_opportunity_pivots.hotel_room_opportunity_id")
						.andOn("hotel_room_opportunity_pivots.language_code", knex.raw("?", [language]))
						.andOnNull("hotel_room_opportunity_pivots.deleted_at")
				})
				// hotel room feature
				.leftJoin("hotel_room_features", function () {
					this.on("hotel_rooms.id", "hotel_room_features.hotel_room_id").andOnNull("hotel_room_features.deleted_at")
				})
				.leftJoin("hotel_room_feature_pivots", function () {
					this.on("hotel_room_features.id", "hotel_room_feature_pivots.hotel_room_feature_id")
						.andOn("hotel_room_feature_pivots.language_code", knex.raw("?", [language]))
						.andOnNull("hotel_room_feature_pivots.deleted_at")
				})

				// Para birimi bilgileri
				.leftJoin("currencies", function () {
					this.on("hotel_room_package_prices.currency_id", "currencies.id").andOnNull("currencies.deleted_at")
				})
			...
			results.forEach((row: any) => {
				...
				if (row.room_image_id) {
					...
					const existingImage = room.images.find((img: any) => img.id === row.room_image_id)
					if (!existingImage) {
						room.images.push({
				...
				if (row.room_opportunity_id) {
					...
					const existingOpportunity = room.opportunities.find((opp: any) => opp.id === row.room_opportunity_id)
				...
				if (row.room_feature_id) {
					...
					const existingFeature = room.features.find((feat: any) => feat.id === row.room_feature_id)
					if (!existingFeature) {
						room.features.push({
