...
export default class DashboardController {
	async index(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const [campaigns, blogs, locations, hotels, tours, carRentals, visas, faqs, activities] = await Promise.all([
	...
	async campaigns(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const { page = 1, limit = 9, service_type } = req.query as { page: number; limit: number; service_type: string }

			const campaignModel = new CampaignModel()
			const blogModel = new BlogModel()

			const [campaignData, blogs] = await Promise.all([campaignModel.getCampaignsPaginated(language, page, limit, service_type), blogModel.getDashboardBlogs(language, 5)])
	...
	async countries(req: FastifyRequest, res: FastifyReply) {
		try {
			const language = (req as any).language
			const countryModel = new CountryModel()
			const countries = await countryModel.getCountries(language)
			return res.status(200).send({
				success: true,
	...
	async subscription(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			sendMail(email, req.t("EMAIL_SUBSCRIPTION.TITLE"), req.t("EMAIL_SUBSCRIPTION.DESCRIPTION"))
	...
	async comments(req: FastifyRequest, res: FastifyReply) {
		try {
			...
			const [hotels, carRentals, activities, tours, visas] = await Promise.all([hotelModel.getComments(language, 3), carRentalModel.getComments(language, 3), activityModel.getComments(language, 3), tourModel.getComments(language, 3), visaModel.getComments(language, 3)])

			const comments = [...hotels, ...carRentals, ...activities, ...tours, ...visas]
