...
export const translateCreate = async ({ target, target_id, target_id_key, data, language_code, trx }: TranslateProps) => {
	try {
		...
		// Use transaction if provided, otherwise use default knex
		const db = trx || knex
		...
		for (const language of languages) {
			...
			let newData = await db(target).insert(translatedData).returning("*")
