...
export async function up(knex: Knex): Promise<void> {
	return knex.schema.createTable("tour_reservations", table => {
		...
		table.uuid("package_id").notNullable().references("id").inTable("tour_packages").onDelete("CASCADE")
		table.uuid("created_by").nullable().references("id").inTable("users").onDelete("CASCADE")
		table.uuid("sales_partner_id").nullable().references("id").inTable("sales_partners").onDelete("CASCADE")
