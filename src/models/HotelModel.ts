...
class HotelModel extends BaseModel {
	...
	static columns = ["id", "location_id", "pool", "private_beach", "transfer", "map_location", "free_age_limit", "solution_partner_id", "star_rating", "status", "admin_approval", "highlight", "comment_count", "average_rating", "refund_days", "created_at", "updated_at", "deleted_at"]

	async getDashboardHotels(language: string, limit: number = 8): Promise<any[]> {
		try {
	...
	private async getHotelsByHighlightStatus(language: string, isHighlighted: boolean, limit?: number): Promise<any[]> {
		try {
			...
			const query = knex("hotels")
				...
				.where(function () {
					this.where("hotel_room_packages.constant_price", true).orWhere(function () {
						this.where("hotel_room_packages.constant_price", false).andWhere(function () {
							this.where("hotel_room_package_prices.start_date", "<=", today).andWhere(function () {
								this.whereNull("hotel_room_package_prices.end_date").orWhere("hotel_room_package_prices.end_date", ">=", today)
							})
						})
					})
				})
          ...
          `,
            ...
            json_build_object(
              ...
              'discount', (array_agg(hotel_room_packages.discount ORDER BY hotel_room_package_prices.main_price ASC))[1]
            ) as package_price
          `)
				)
	...
	getComments(language: string, limit: number = 3, id?: string): Promise<any[]> {
		return knex("comments")
			...
			.leftJoin("hotel_pivots", function () {
				this.on("comments.service_id", "hotel_pivots.hotel_id").andOn("hotel_pivots.language_code", knex.raw("?", [language]))
			})
			.whereNull("hotel_pivots.deleted_at")
