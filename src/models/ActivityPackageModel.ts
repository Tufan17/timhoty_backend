...
class ActivityPackageModel extends BaseModel {
	...
	static columns = ["id", "activity_id", "return_acceptance_period", "discount", "total_tax_amount", "constant_price", "date", "created_at", "updated_at", "deleted_at"]
	...
	async getPackagesByActivityId(activityId: string) {
		return await knex("activity_packages").select("activity_packages.*", "activities.*").join("activities", "activity_packages.activity_id", "activities.id").where("activity_packages.activity_id", activityId).whereNull("activity_packages.deleted_at").whereNull("activities.deleted_at").orderBy("activity_packages.date", "asc")
	}

	async getPackagesByDateRange(startDate: string, endDate: string) {
		return await knex("activity_packages").select("activity_packages.*", "activities.*").join("activities", "activity_packages.activity_id", "activities.id").whereBetween("activity_packages.date", [startDate, endDate]).whereNull("activity_packages.deleted_at").whereNull("activities.deleted_at").orderBy("activity_packages.date", "asc")
	}
