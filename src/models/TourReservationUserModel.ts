...
class TourReservationUserModel extends BaseModel {
  ...
  async deleteByReservationId(tourReservationId: string) {
    await knex("tour_reservation_users")
      .where("tour_reservation_id", tourReservationId)
      .whereNull("deleted_at")
  ...
  async getReservationUserStats(tourReservationId: string) {
    const total = await knex("tour_reservation_users")
      .where("tour_reservation_id", tourReservationId)
      .whereNull("deleted_at")
      .count("id as total")
      .first();

    const adults = await knex("tour_reservation_users")
      .where("tour_reservation_id", tourReservationId)
      .where("type", "adult")
      .whereNull("deleted_at")
      .count("id as adults")
    ...
    const children = await knex("tour_reservation_users")
      .where("tour_reservation_id", tourReservationId)
      .where("type", "child")
      .whereNull("deleted_at")
