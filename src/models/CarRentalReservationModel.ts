...
class CarRentalReservationModel extends BaseModel {
	...
	static columns = ["id", "car_rental_id", "package_id", "created_by", "sales_partner_id", "progress_id", "price", "currency_code", "payment_id", "different_invoice", "status", "start_date", "end_date", "created_at", "updated_at", "deleted_at"]

	async getReservationByCarRentalId(carRentalId: string) {
		return await knex("car_rental_reservations").where("car_rental_id", carRentalId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByPackageId(packageId: string) {
		return await knex("car_rental_reservations").where("package_id", packageId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByUserId(userId: string) {
		return await knex("car_rental_reservations").where("created_by", userId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationBySalesPartnerId(salesPartnerId: string) {
		return await knex("car_rental_reservations").where("sales_partner_id", salesPartnerId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByPaymentId(paymentId: string) {
		return await knex("car_rental_reservations").where("payment_id", paymentId).whereNull("deleted_at").first()
	}

	async updatePaymentId(id: string, paymentId: string) {
		return await knex("car_rental_reservations").where("id", id).whereNull("deleted_at").update({
	...
	async getReservationWithDetails(reservationId: string) {
		return await knex("car_rental_reservations")
			...
			.leftJoin("car_rental_pivots", function () {
				this.on("car_rentals.id", "=", "car_rental_pivots.car_rental_id").andOn("car_rental_pivots.language_code", "=", "en")
			})
			...
			.leftJoin("car_rental_package_pivots", function () {
				this.on("car_rental_packages.id", "=", "car_rental_package_pivots.car_rental_package_id").andOn("car_rental_package_pivots.language_code", "=", "en")
			})
			.leftJoin("users", "car_rental_reservations.created_by", "users.id")
			.leftJoin("sales_partners", "car_rental_reservations.sales_partner_id", "sales_partners.id")
			.select("car_rental_reservations.*", "car_rental_pivots.title as car_rental_title", "car_rental_package_pivots.name as package_name", "users.first_name", "users.last_name", "users.email as user_email", "sales_partners.company_name")
	...
	async getReservationStats() {
		const total = await knex("car_rental_reservations").whereNull("deleted_at").count("id as total").first()

		const active = await knex("car_rental_reservations").where("status", true).whereNull("deleted_at").count("id as active").first()

		const pending = await knex("car_rental_reservations").where("status", false).whereNull("deleted_at").count("id as pending").first()

		const withPayment = await knex("car_rental_reservations").whereNotNull("payment_id").whereNull("deleted_at").count("id as with_payment").first()
	...
	async getUserReservationById(reservationId: string, language: string) {
		...
		return await knex("car_rental_reservations")
			.select(
				...
				"country_pivots.name as car_rental_country",
				"car_rental_package_pivots.name as package_name",
				"car_rental_packages.return_acceptance_period as package_return_acceptance_period",
				// 1 tane fotoğraf gelsin: subquery ile ilk fotoğrafı alıyoruz
				knex.raw(`(
          SELECT image_url
          FROM car_rental_galleries
          ...
          LIMIT 1
            ...
            LIMIT 1
				...
				knex.raw(`(
            SELECT to_jsonb(cri)
            FROM car_rental_reservation_invoices cri
            WHERE cri.car_rental_reservation_id = car_rental_reservations.id
			...
			.leftJoin("car_rental_reservation_users", "car_rental_reservations.id", "car_rental_reservation_users.car_rental_reservation_id")
			.whereNull("car_rental_reservation_users.deleted_at")
			.groupBy("car_rental_reservations.id", "car_rental_pivots.title", "city_pivots.name", "country_pivots.name", "car_rental_package_pivots.name", "car_rental_packages.return_acceptance_period")
			.orderBy("car_rental_reservations.created_at", "desc")
			.first()
	}
}
