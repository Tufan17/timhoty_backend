...
class ActivityReservationModel extends BaseModel {
  ...
  async getReservationByHourId(hourId: string) {
    return await knex("activity_reservations")
      .where("activity_package_hour_id", hourId)
      .whereNull("deleted_at")
      .orderBy("created_at", "desc");
  }
  ...
  async getReservationStats() {
    const total = await knex("activity_reservations")
      .whereNull("deleted_at")
      .count("id as total")
      .first();

    const active = await knex("activity_reservations")
      .where("status", true)
      .whereNull("deleted_at")
      .count("id as active")
      .first();

    const pending = await knex("activity_reservations")
      .where("status", false)
      .whereNull("deleted_at")
      .count("id as pending")
      .first();

    const withPayment = await knex("activity_reservations")
  ...
  async getUserReservation(userId: string, language: string) {
   try{
    ...
    .whereNull("activity_reservations.deleted_at")
    .leftJoin("activity_pivots", function () {
      this.on("activity_reservations.activity_id", "=", "activity_pivots.activity_id").andOn(
        "activity_pivots.language_code",
        "=",
        knex.raw("?", [language])
      );
    ...
    .orderBy("activity_reservations.created_at", "desc");  }catch(error){
