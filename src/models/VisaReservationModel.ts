...
class VisaReservationModel extends BaseModel {
	...
	static columns = ["id", "visa_id", "package_id", "created_by", "sales_partner_id", "progress_id", "price", "currency_code", "payment_id", "different_invoice", "status", "date", "created_at", "updated_at", "deleted_at"]

	async getReservationByVisaId(visaId: string) {
		return await knex("visa_reservations").where("visa_id", visaId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByPackageId(packageId: string) {
		return await knex("visa_reservations").where("package_id", packageId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByUserId(userId: string) {
		return await knex("visa_reservations").where("created_by", userId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationBySalesPartnerId(salesPartnerId: string) {
		return await knex("visa_reservations").where("sales_partner_id", salesPartnerId).whereNull("deleted_at").orderBy("created_at", "desc")
	}

	async getReservationByPaymentId(paymentId: string) {
		return await knex("visa_reservations").where("payment_id", paymentId).whereNull("deleted_at").first()
	}

	async updatePaymentId(id: string, paymentId: string) {
		return await knex("visa_reservations").where("id", id).whereNull("deleted_at").update({
	...
	// 			this.on("visas.id", "=", "visa_pivots.visa_id").andOn("visa_pivots.language_code", "=", "en")
	// 		})
	...
	// 		.select("visa_reservations.*", "visa_pivots.title as visa_title", "visa_package_prices.price as package_price", "currencies.code as currency_code", "currencies.symbol as currency_symbol", "users.first_name", "users.last_name", "users.email as user_email", "sales_partners.company_name")
	...
	async getReservationWithDetails(reservationId: string, language: string = "en") {
		return await knex("visa_reservations")
			.select(
				...
				"visa_package_prices.main_price as package_main_price",
				"visa_package_prices.child_price as package_child_price",
			...
			.where("visa_reservations.id", reservationId)
			.whereNull("visa_reservations.deleted_at")
			.groupBy("visa_reservations.id", "visa_pivots.title", "visa_pivots.refund_policy", "visa_package_prices.main_price", "visa_package_prices.child_price", "visas.refund_days", "currencies.code", "currencies.symbol", "users.name_surname", "users.email", "country_pivots.name")
			.first()
	}
	...
	async getReservationStats() {
		const total = await knex("visa_reservations").whereNull("deleted_at").count("id as total").first()

		const active = await knex("visa_reservations").where("status", true).whereNull("deleted_at").count("id as active").first()

		const pending = await knex("visa_reservations").where("status", false).whereNull("deleted_at").count("id as pending").first()

		const withPayment = await knex("visa_reservations").whereNotNull("payment_id").whereNull("deleted_at").count("id as with_payment").first()
	...
	async getUserReservation(userId: string, language: string) {
		return await knex("visa_reservations")
          ...
          SELECT to_jsonb(c)
          FROM comments c
          WHERE c.reservation_id = visa_reservations.id
	...
	async getSalesPartnerReservationById(reservationId: string, language: string) {
		return await knex("visa_reservations")
			...
			.select(
				...
				knex.raw("COALESCE(json_agg(DISTINCT jsonb_build_object('id', visa_reservation_users.id, 'name', visa_reservation_users.name, 'surname', visa_reservation_users.surname, 'email', visa_reservation_users.email, 'phone', visa_reservation_users.phone, 'type', visa_reservation_users.type, 'age', visa_reservation_users.age)) FILTER (WHERE visa_reservation_users.id IS NOT NULL), '[]'::json) as users")
			)
			.where("visa_reservations.id", reservationId)
			.whereNull("visa_reservations.deleted_at")
			.whereNull("visa_reservation_users.deleted_at")
			.groupBy("visa_reservations.id", "visa_pivots.title", "visa_pivots.general_info", "visa_pivots.visa_info", "visa_pivots.refund_policy", "visa_package_prices.main_price", "visa_package_prices.child_price", "currencies.code", "currencies.symbol", "users.name_surname", "users.email", "users.phone", "country_pivots.name")
			.first()
	}
}
