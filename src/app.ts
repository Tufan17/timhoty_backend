...
// Viator API Routes
import viatorRoutes from "./routes/v2/viator"
...
export default async function app(fastify: FastifyInstance) {
	...
	// VIATOR API ROUTES
	// ===========================================
	fastify.register(viatorRoutes, { prefix: "/api/v2/viator" })
	...
	fastify.get("/test-email", async (request, reply) => {
		...
		await sendMail("alitufan.asidev@gmail.com", "Test Email", html)
		return reply.send({
			success: true,
	...
	fastify.get("/uploads/:folder/:filename", async function handler(request, reply) {
		try {
